...
def audio_tagging_results(reference, estimated):
    ...
    if "event_label" in reference.columns:
        ...
        reference['event_label'], _ = utils.encode_labels(
            reference['event_label'], encoder=encoder)
        estimated['event_label'], _ = utils.encode_labels(
            estimated['event_label'], encoder=encoder)
...
def mAP(y_true, y_pred):
    return skmetrics.average_precision_score(y_true, y_pred, 'macro')
