...
class ASMappingEvalDataset(Dataset):
...
class ASMappingWeakDataset(Dataset):
    ...
    def __init__(self,
        ...
        """
        min_sim_percent: percentile to automatically determine the threshold, for example phrase like 'loudly' will ideally be very distant to all phrases, so a percentile like 97 can be used to exclude such phrases
        """
        self.aid_to_h5 = load_dict_from_csv(waveform, ("audio_id", "hdf5_path"))
        self.cache = {}
        self.data = json.load(open(label))
...
class ASMappingEvalLabelSimDataset(Dataset):
...
class ASMappingStrongDataset(ASMappingWeakDataset):
    ...
    def __init__(self,
        ...
        super().__init__(waveform=waveform,
                         label=label,
                         audioset_label=audioset_label,
                         phrase_embed=phrase_embed,
                         as_label_embed=as_label_embed,
                         label_encoder=label_encoder,
                         thresholds=thresholds,
                         min_sim_percent=None,
                         use_audioset_label=use_audioset_label,
                         topk=topk,
...
class KmeansMappingEvalDataset(Dataset):
...
class KmeansMappingDataset(Dataset):
...
class SpectralMappingEvalDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.cache = {}
        self.data = json.load(open(label))
        self.cluster_map = json.load(open(cluster_map))
        self.phrase_to_idx = {}
        for phrase_idx, phrases in self.cluster_map.items():
            for phrase in phrases:
                self.phrase_to_idx[phrase] = int(phrase_idx)
        self.generate_index()
    ...
    def __getitem__(self, index):
        ...
        output = {
            ...
            "text": caption,
            "waveform": waveform,
            "text_idx": self.phrase_to_idx[phrase_item["phrase"]],
            "start_index": phrase_item["start_index"],
            "end_index": phrase_item["end_index"],
        }
...
class SpectralMappingDataset(Dataset):
    ...
    def __init__(self,
        ...
        for phrase_idx, phrases in self.cluster_map.items():
            for phrase in phrases:
                self.phrase_to_idx[phrase] = int(phrase_idx)
        self.classes_num = len(self.cluster_map.keys())
        self.time_resolution = time_resolution
        self.sample_rate = sample_rate
        self.no_waveform = no_waveform
        if max_audio_length is not None:
    ...
    def __getitem__(self, index):
        ...
        if self.label_type == "weak":
            ...
            for phrase_item in audio_item["phrases"]:
                phrase = phrase_item["phrase"]
                label[self.phrase_to_idx[phrase]] = 1
            output["label"] = label
        elif self.label_type == "strong":
            audio_duration = waveform.shape[0] / self.sample_rate
            n_frame = math.floor(audio_duration / self.time_resolution) + 1
            ...
            for phrase_item in audio_item["phrases"]:
                phrase = phrase_item["phrase"]
                label_idx = self.phrase_to_idx[phrase]
                weak_label[label_idx] = 1
                for start, end in phrase_item["segments"]:
                    onset = round(start / self.time_resolution)
                    offset = round(end / self.time_resolution)
